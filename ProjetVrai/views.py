...
def login(requete):
    if len(requete.POST)>0:
        ...
        if numero=="admi" and password=='2678':
...
def admini(requete):
    if 'ajoutMat' in requete.POST:
        #numMat=requete.POST['numMat']
        mate = materiel(numMat=requete.POST['numero'],design=requete.POST['design'],
                        Pu=requete.POST['pu'], Stock=requete.POST['stock'], image='media/'+requete.FILES['image'].name)
        mate.save()
        return redirect("/administrateur")
    else:
        if 'modifMat' in requete.POST:
            mod=materiel.objects.get(id=requete.POST['id'])
            mod.numMat=requete.POST['numero']
            mod.design=requete.POST['design']
            mod.Pu=requete.POST['pu']
            mod.Stock=requete.POST['stock']
            
            mod.save()
            return redirect("/administrateur")
        else:
            mat=materiel.objects.all()
            logged_user=requete.session["logged_user_id"]="admin"
            return render(requete,'administrateur/index.html',{'logged_user':logged_user,'materiels':mat,
                                                       'lenMat':len(mat)})
def ajax_formMat(requete):
    html_to_return=""
    if 'value' in requete.GET:
        idm=requete.GET["value"]
        sauve=materiel.objects.get(id=idm)
        sauve.delete()
